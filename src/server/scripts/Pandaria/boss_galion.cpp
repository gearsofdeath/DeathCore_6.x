...
enum Spells
{
    SPELL_EARTHQUAKE        = 144695,
    SPELL_SUNDER_ARMOR      = 144689,
    SPELL_CHAIN_LIGHTNING   = 144696,
    SPELL_OVERRUN           = 144688,
    SPELL_ENRAGE            = 144692,
...
class boss_galion : public CreatureScript
{
    public:
        ...
        struct boss_galionAI : public ScriptedAI
        {
            boss_galionAI(Creature* creature) : ScriptedAI(creature)
            {
                Initialize();
            }
        ...
        {
            return new boss_galionAI(creature);
        }
};
